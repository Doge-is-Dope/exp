...
For architecture assessments or changes to responsibility or contract boundaries, read [architecture decisions and verification](../exp-build/references/architecture.md). Trace a confirmed next change through the code when one exists; otherwise review current responsibilities and testability. Report current defects, evidence gaps, and optional future improvements separately. File counts and preferred patterns do not establish defects.
